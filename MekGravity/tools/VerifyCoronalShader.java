...
/** Actual coronal shader and bounded original geometry, in a hidden OpenGL context. */
public class VerifyCoronalShader {
    ...
    private static ByteBuffer field(double phase,float strength){var bytes=BufferUtils.createByteBuffer(128*4*24);
        ...
        CoronalField.emit(false,(x,y,z,angle,across)->{bytes.putFloat(x).putFloat(y).putFloat(z-1.5F).putFloat(angle).putFloat(across);bytes.put((byte)180).put((byte)sin).put((byte)cos).put((byte)(strength*220));});return bytes.flip();
    }
    ...
    public static void main(String[] args)throws Exception{
        ...
        try{glfwMakeContextCurrent(window);GL.createCapabilities();System.out.println("OpenGL: "+glGetString(GL_VERSION));String fog;
            ...
            {String name="coronal_processing";Path dir=root.resolve("src/main/resources/assets/mekgravity/shaders/core");
                ...
                var active=frame(0,1,false);var moving=frame(40,1,false);var idle=frame(0,0,false);var blocked=frame(0,1,true);save(active,out.resolve(name+".png"));save(moving,out.resolve(name+"-moving.png"));save(idle,out.resolve(name+"-idle.png"));
                ...
                int quads=(field(0,1).remaining()/24)/4;System.out.println("PASS "+name+": actual GLSL, moving/idle pixels, depth occlusion; "+quads+" quads");
