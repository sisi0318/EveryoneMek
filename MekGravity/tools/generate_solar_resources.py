...
'''
    ...
    z.update({'load_duration':'设定负载续航 %s时 %s分 %s秒','build_at':'%s · %s','material_count':'%s × %s','facing':'聚束器需要朝向恒星胚核'})
    en.update({'load_duration':'At set load: %sh %sm %ss','build_at':'%s · %s','material_count':'%s × %s','facing':'Aim the focus at the stellar seed'})
