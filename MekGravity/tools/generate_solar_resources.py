...
'''
    ...
    z.update({'heat_warning':'警告：恒星尚未熄火，请保持距离','heat_burning':'高温灼伤！立即远离恒星胚核','heat_contact':'危险：已接触恒星！立即撤离','load_duration':'设定负载续航 %s时 %s分 %s秒','build_at':'%s · %s','material_count':'%s × %s','facing':'聚束器需要朝向恒星胚核'})
    en.update({'heat_warning':'Warning: stellar core is still hot. Keep your distance.','heat_burning':'Scorching heat! Move away from the stellar core!','heat_contact':'DANGER: stellar contact! Retreat immediately!','load_duration':'At set load: %sh %sm %ss','build_at':'%s · %s','material_count':'%s × %s','facing':'Aim the focus at the stellar seed'})
