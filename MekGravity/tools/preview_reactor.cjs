...
for(let x=0;x<7;x++)for(let y=0;y<7;y++)for(let z=0;z<7;z++){
  ...
  const name={frame:'frame_formed',casing:'casing_formed',controller:'reactor_formed',fuel:'fuel_formed',excitation:'energy_formed',output:'energy_formed_output',coil:'basic_coil_formed_active',core:'core_active'}[kind];
  if(!name)continue;
