...
public record OrbitalRecipe(String machine,List<Input> inputs,ItemStack result,int ticks,long energy,int tier) implements Recipe<OrbitalRecipe.Inventory> {
    ...
    public OrbitalRecipe{if(!(machine.equals("gravity_forge")||machine.equals("flare_captor"))||inputs.isEmpty()||inputs.size()>4||result.isEmpty()||result.getCount()>64||ticks<1||ticks>72000||energy<1||energy>100_000_000_000_000L||tier<0||tier>3)throw new IllegalArgumentException("Invalid orbital recipe");inputs=List.copyOf(inputs);result=result.copy();}
    ...
    @Override public RecipeType<?> getType(){return ModuleContent.TYPE.get();}
    @Override public RecipeSerializer<?> getSerializer(){return ModuleContent.SERIALIZER.get();}
    ...
    public static final class Serializer implements RecipeSerializer<OrbitalRecipe>{
        private static final MapCodec<OrbitalRecipe> CODEC=RecordCodecBuilder.mapCodec(i->i.group(Codec.STRING.fieldOf("machine").forGetter(OrbitalRecipe::machine),Input.CODEC.listOf().fieldOf("inputs").forGetter(OrbitalRecipe::inputs),ItemStack.STRICT_CODEC.fieldOf("result").forGetter(OrbitalRecipe::result),Codec.intRange(1,72000).fieldOf("ticks").forGetter(OrbitalRecipe::ticks),Codec.LONG.validate(n->n>=1&&n<=100_000_000_000_000L?DataResult.success(n):DataResult.error(()->"Energy out of range")).fieldOf("energy").forGetter(OrbitalRecipe::energy),Codec.intRange(0,3).optionalFieldOf("tier",0).forGetter(OrbitalRecipe::tier)).apply(i,OrbitalRecipe::new));
        private static final StreamCodec<RegistryFriendlyByteBuf,OrbitalRecipe> STREAM=StreamCodec.composite(ByteBufCodecs.STRING_UTF8,OrbitalRecipe::machine,Input.STREAM.apply(ByteBufCodecs.list(4)),OrbitalRecipe::inputs,ItemStack.STREAM_CODEC,OrbitalRecipe::result,ByteBufCodecs.VAR_INT,OrbitalRecipe::ticks,ByteBufCodecs.VAR_LONG,OrbitalRecipe::energy,ByteBufCodecs.VAR_INT,OrbitalRecipe::tier,OrbitalRecipe::new);
        public MapCodec<OrbitalRecipe> codec(){return CODEC;}public StreamCodec<RegistryFriendlyByteBuf,OrbitalRecipe> streamCodec(){return STREAM;}
    }
}
