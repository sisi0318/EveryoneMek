...
public final class NodeStorage {
    ...
    public static boolean transfer(OrbitalModule sender,OrbitalModule receiver,FieldSource source,int channel){var from=sender.node;var to=receiver.node;boolean moved=false;
        if(channel==1&&sender.channel(1)&&receiver.channel(1)&&!from.energyIn.isEmpty()){
            ...
            else{from.energyIn.extract(amount,Action.EXECUTE,AutomationType.INTERNAL);if(to.energyOut.insert(amount,Action.EXECUTE,AutomationType.INTERNAL)!=0)throw new IllegalStateException("Reserved energy capacity changed");from.energyMoved=add(from.energyMoved,amount);sender.paidEnergy+=ModuleConfig.ENERGY_PACKET_COST.get();moved=true;}
        }
        if(channel==2&&sender.channel(2)&&receiver.channel(2))for(var tank:from.fluidIn){if(tank.isEmpty())continue;int room=fluidRoom(to,tank.getFluid());long rate=ModuleConfig.FLUID_COST.get();int amount=(int)Math.min(Math.min(tank.getFluidAmount(),room),source.available()/rate);
            ...
            insertFluid(to,tank.extract(amount,Action.EXECUTE,AutomationType.INTERNAL));from.fluidMoved+=amount;sender.paidEnergy+=cost;moved=true;
        }
        if(channel==3&&sender.channel(3)&&receiver.channel(3))for(var tank:from.chemicalIn){if(tank.isEmpty())continue;long room=chemicalRoom(to,tank.getStack()),rate=ModuleConfig.CHEMICAL_COST.get(),amount=Math.min(Math.min(tank.getStored(),room),source.available()/rate);
            ...
            insertChemical(to,tank.extract(amount,Action.EXECUTE,AutomationType.INTERNAL));from.chemicalMoved+=amount;sender.paidEnergy+=cost;moved=true;
        }return moved;
    }
    ...
    public void eject(OrbitalModule owner){if(!owner.autoEject)return;var level=owner.getLevel();var pos=owner.getBlockPos().relative(owner.getDirection());if(!level.hasChunkAt(pos))return;var side=owner.getDirection().getOpposite();
        if(!energyOut.isEmpty()){
        ...
        var fluid=level.getCapability(Capabilities.FluidHandler.BLOCK,pos,side);if(fluid!=null)for(var tank:fluidOut){if(tank.isEmpty())continue;int accepted=fluid.fill(tank.getFluid().copy(),FluidAction.EXECUTE);if(accepted>0)tank.extract(accepted,Action.EXECUTE,AutomationType.INTERNAL);}
        var chemical=level.getCapability(mekanism.common.capabilities.Capabilities.CHEMICAL.block(),pos,side);if(chemical!=null)for(var tank:chemicalOut){if(tank.isEmpty())continue;long accepted=tank.getStored()-chemical.insertChemical(tank.getStack().copy(),Action.EXECUTE).getAmount();if(accepted>0)tank.extract(accepted,Action.EXECUTE,AutomationType.INTERNAL);}
    }
}
