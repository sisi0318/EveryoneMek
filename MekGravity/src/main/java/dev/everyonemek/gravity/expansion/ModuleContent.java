...
public final class ModuleContent {
    ...
    static{for(var kind:ModuleKind.values()){
        ...
        TILE.put(kind,TILES.<OrbitalModule>mekBuilder(block,(pos,state)->kind==ModuleKind.NODE?new NodeModule(pos,state):kind==ModuleKind.OBSERVATORY?new OrbitalModule(pos,state):new ProcessModule(pos,state)).clientTicker(TileEntityMekanism::tickClient).serverTicker(TileEntityMekanism::tickServer).build());
    }}
