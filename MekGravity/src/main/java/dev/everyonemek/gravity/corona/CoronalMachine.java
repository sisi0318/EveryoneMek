...
public final class CoronalMachine extends TileEntityMekanism {
    public List<CoronalInventorySlot> inputs;public List<CoronalInventorySlot> outputs;
    ...
    @Override public void onContentsChanged(){super.onContentsChanged();nextAttempt=0;}
    ...
    @Override protected IInventorySlotHolder getInitialInventory(IContentsListener listener){
        ...
        for(int i=0;i<9;i++){var slot=new CoronalInventorySlot(false,listener,12+i%3*18,36+i/3*18);inputs.add(slot);holder.addSlot(slot,RelativeSide.TOP,RelativeSide.BOTTOM,RelativeSide.LEFT,RelativeSide.RIGHT);}
        for(int i=0;i<9;i++){var slot=new CoronalInventorySlot(true,listener,154+i%3*18,36+i/3*18);outputs.add(slot);holder.addSlot(slot,RelativeSide.FRONT);}return holder.build();
    }
    ...
    private int outputRoom(ItemStack item){
        int room=0;for(var slot:outputs){var s=slot.getStack();if(s.isEmpty()||ItemStack.isSameItemSameComponents(s,item))room+=Math.max(0,slot.getLimit(item)-s.getCount());}return room;
    }
    public static int workTicks(int base,int tier){int speed=SolarConfig.CORONAL_ACCELERATION.get()*(1<<Math.clamp(tier,0,3));return Math.max(1,(base+speed-1)/speed);}
    private record Plan(ItemStack result,ItemStack sample,int amount,int batch,int ticks,long energy,int[] consumed){}
    private Plan plan(SolarController c){var view=snapshot();boolean tierBlocked=false,outputBlocked=false,energyBlocked=false;
        long availableEnergy=Math.max(0,c.stored-c.reserve());
        // One transaction per batch; output and available energy bound matching before the flow search.
        for(var holder:level.getRecipeManager().getAllRecipesFor(CoronalContent.TYPE.get())){
            ...
            int room=outputRoom(recipe.result())/recipe.result().getCount();if(room==0){outputBlocked=true;continue;}
            long funded=availableEnergy/recipe.energy();if(funded==0){energyBlocked=true;continue;}
        ...
        for(int i=0;i<view.size();i++){var stack=view.getItem(i);if(stack.isEmpty())continue;var input=new SingleRecipeInput(stack.copyWithCount(1));
            ...
            int room=outputRoom(result)/result.getCount();if(room==0){outputBlocked=true;continue;}
            long funded=availableEnergy/SolarConfig.CORONAL_ENERGY.get();if(funded==0){energyBlocked=true;continue;}
    ...
    private void finish(){
        ...
        markForSave();if(remaining==0){product=sample=ItemStack.EMPTY;}else status="output_full";
    }
    private void process(SolarController c){
        ...
        try{
            ...
            if(remaining==0){if(level.getGameTime()<nextAttempt)return;nextAttempt=level.getGameTime()+5;if(inputs.stream().allMatch(s->s.isEmpty())){status="no_recipe";return;}var plan=plan(c);if(plan==null)return;if(!c.spendForCorona(plan.energy)){status="energy";return;}
                ...
                product=plan.result;sample=plan.sample;remaining=plan.amount;batch=plan.batch;duration=plan.ticks;progress=0;paidEnergy=plan.energy;markForSave();
            }
            working=true;status="running";progress++;markForSave();visual(true,false);if(progress>=duration){finish();eject();nextAttempt=0;}
    ...
    private void eject(){if(!autoEject)return;var front=worldPosition.relative(getDirection());if(!level.hasChunkAt(front))return;
        ...
        int budget=4096;for(var slot:outputs){if(budget<=0)break;if(slot.isEmpty())continue;var offer=slot.getStack().copyWithCount(Math.min(budget,slot.getCount()));int accepted=offer.getCount()-ItemHandlerHelper.insertItemStacked(target,offer,true).getCount();if(accepted<=0)continue;
            var sent=slot.extractItem(accepted,Action.EXECUTE,AutomationType.INTERNAL);var rest=ItemHandlerHelper.insertItemStacked(target,sent,false);budget-=sent.getCount()-rest.getCount();if(!rest.isEmpty())slot.insertItem(rest,Action.EXECUTE,AutomationType.INTERNAL);
        }
    }
