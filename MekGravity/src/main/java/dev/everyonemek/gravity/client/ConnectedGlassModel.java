...
public final class ConnectedGlassModel extends BakedModelWrapper<BakedModel> {
    ...
    private ConnectedGlassModel(BakedModel original,Map<Direction,List<List<BakedQuad>>> faces){super(original);this.faces=faces;}
    ...
    public static void additional(ModelEvent.RegisterAdditional event){for(var face:Direction.values())for(int part=0;part<8;part++)event.register(id(face,part));}
    ...
    public static void bake(ModelEvent.ModifyBakingResult event){
        ...
        for(var state:Content.PARTS.get(PartBlock.Kind.GLASS).get().getStateDefinition().getPossibleStates()){
            ...
            if(original!=null)event.getModels().put(key,wrappers.computeIfAbsent(original,m->new ConnectedGlassModel(m,immutable)));
        }
    }
