...
public final class SolarWindow extends GuiWindow {
    ...
    public SolarWindow(IGuiWrapper gui,SolarController c,SolarMenu m,int page){super(gui,18,22,204,page==0?240:page==1?208:page==2?205:120,WindowType.UNSPECIFIED);this.page=page;
        ...
        if(page==1){addChild(new GuiInnerScreen(gui,relativeX+10,relativeY+27,184,103,()->{
            ...
            lines.add(!m.formed?Content.text("syncing"):SolarContent.text(m.constraint<m.collector?"limit_constraint":m.collector<m.constraint?"limit_collector":"limit_balanced"));
            lines.add(m.formed?SolarContent.text("ready"):m.errorPos==null?SolarContent.text("structure"):Content.text("error_at",m.errorPos.toShortString()));return lines;
        }).spacing(0));button(gui,m,10,139,88,"preview",2);button(gui,m,106,139,88,"build",3);
            addChild(new MekanismButton(gui,relativeX+10,relativeY+163,184,16,Content.text("build_grade",Content.text("grade."+c.buildTier)),(b,x,y)->send(m,20)){@Override public void tick(){super.tick();setMessage(Content.text("build_grade",Content.text("grade."+c.buildTier)));}});
            button(gui,m,10,186,184,"upgrade_assembly",21);}
        if(page==2){
        ...
        if(page==2)addChild(new GuiInnerScreen(gui,relativeX+10,relativeY+134,184,57,()->List.of(
