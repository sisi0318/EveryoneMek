...
public final class SolarContent {
    ...
    static{
        ...
        TABS.register("solar",()->CreativeModeTab.builder().title(text("tab")).icon(()->new ItemStack(CONTROLLER)).displayItems((p,out)->{out.accept(CONTROLLER);PARTS.values().forEach(out::accept);out.accept(COMPRESSED.get());out.accept(PREFORM.get());out.accept(FUEL.get());}).build());
    }
