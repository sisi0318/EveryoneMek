...
public final class SolarClient {
    @SubscribeEvent public static void screens(RegisterMenuScreensEvent e){e.register(dev.everyonemek.gravity.expansion.ModuleContent.PANEL_MENU.get(),dev.everyonemek.gravity.expansion.LinkPanelScreen::new);e.register(dev.everyonemek.gravity.expansion.ModuleContent.MENU.get(),dev.everyonemek.gravity.expansion.ModuleScreen::new);e.register(dev.everyonemek.gravity.corona.CoronalContent.MENU.get(),dev.everyonemek.gravity.corona.CoronalScreen::new);e.register(SolarContent.MENU.get(),SolarScreen::new);e.register(SolarContent.FUEL_MENU.get(),SolarFuelScreen::new);}
    ...
    @SubscribeEvent public static void setup(net.neoforged.fml.event.lifecycle.FMLClientSetupEvent e){e.enqueueWork(()->{dev.everyonemek.gravity.expansion.LinkPanelNetwork.clientReceiver=p->{var mc=net.minecraft.client.Minecraft.getInstance();if(mc.player!=null&&mc.player.containerMenu instanceof dev.everyonemek.gravity.expansion.LinkPanelMenu menu)menu.accept(p);};net.minecraft.client.renderer.item.ItemProperties.register(SolarContent.block(SolarBlock.Kind.ENERGY,0).asItem(),net.minecraft.resources.ResourceLocation.fromNamespaceAndPath(MekGravity.ID,"output"),(s,l,p,seed)->{var t=s.get(SolarContent.STOCK.get());return t!=null&&t.getBoolean("output")?1:0;});});}
}
