...
public final class Controller extends TileEntityMekanism {
    ...
    public long stored,fuelRemaining,fuelTotal,gross,selfUse,heatSpent,lastOutput;
    ...
    private long clock=Long.MIN_VALUE,exported;
    ...
    public void clock(){if(level.getGameTime()!=clock){lastOutput=exported;exported=0;clock=level.getGameTime();}}
    public long accept(long n,boolean simulate){if(n<=0||!structure.valid())return 0;long accepted=Math.min(n,Math.max(0,capacity()-stored));if(!simulate&&accepted>0){stored+=accepted;markForSave();}return accepted;}
    public long extract(long n,boolean simulate){if(n<=0||!ignited||!structure.valid())return 0;clock();long amount=Math.min(n,Math.min(Math.max(0,stored-reserve()),Math.max(0,structure.grade.power()-exported)));if(!simulate&&amount>0){stored-=amount;exported+=amount;markForSave();}return amount;}
    ...
    public CooledCoolant coolant(){if(cold.isEmpty()||!cold.is(MekanismChemicals.SODIUM)||cold.isRadioactive())return null;return cold.getData(IMekanismDataMapTypes.INSTANCE.cooledChemicalCoolant());}
    private static long coolantFor(long power){if(power<=0)return 0;long rate=ReactorConfig.COOLANT_PER_GJ.get();return power/1_000_000_000L*rate+(power%1_000_000_000L*rate+999_999_999L)/1_000_000_000L;}
    private static long heatFor(long power,CooledCoolant coolant){
        var heated=coolant.heat(1).getData(IMekanismDataMapTypes.INSTANCE.heatedChemicalCoolant());
        if(heated==null||heated.otherVariant().value()!=MekanismChemicals.SODIUM.get())return Long.MAX_VALUE;
        // The boiler uses the hot variant's enthalpy. Pay the larger value if a data pack changes one side.
        return mekanism.api.math.MathUtils.ceilToLong(coolantFor(power)*Math.max(coolant.thermalEnthalpy(),heated.thermalEnthalpy()));
    }
    public void react(){
        long previousGross=gross;
        gross=selfUse=heatSpent=0;
        ...
        var coolant=coolant();if(coolant==null||coolant.heat(1).isRadioactive()){status="cold_missing";return;}
        if(heatFor(1,coolant)==Long.MAX_VALUE){status="coolant_invalid";return;}
        if(!hot.isEmpty()&&!ChemicalStack.isSameChemical(hot,coolant.heat(1))){status="hot_blocked";return;}
        long cooling=Math.min(cold.getAmount(),Math.max(0,tankCapacity()-hot.getAmount()));if(cooling<=0){status="hot_blocked";return;}
        ...
        long available=Math.min(requested,Math.min(room,cooling*1_000_000_000L/ReactorConfig.COOLANT_PER_GJ.get()));
        if(available<50){status=room<50?"full":"cold_missing";return;}
        // Prepaid fuel is a finite energy budget. Coolant enthalpy is paid separately from electrical generation.
        long heat=heatFor(available,coolant);if(heat<=0||heat>Long.MAX_VALUE-available){status="coolant_invalid";return;}
        chargeFuel(available+heat);
        if(fuelRemaining<available+heat){long lo=0,hi=available;while(lo<hi){long mid=lo+(hi-lo+1)/2;long h=heatFor(mid,coolant);if(h>=0&&h<=fuelRemaining&&mid<=fuelRemaining-h)lo=mid;else hi=mid-1;}available=lo;}
        if(available<50){status="fuel_missing";return;}
        long sodium=coolantFor(available);heat=heatFor(available,coolant);
        if(sodium>cooling||available>fuelRemaining-heat)throw new IllegalStateException("Reactor transaction preflight changed");
        gross=available;selfUse=(gross+49)/50;heatSpent=heat;
        fuelRemaining-=gross+heat;stored+=gross-selfUse;
        cold=cold.copyWithAmount(cold.getAmount()-sodium);hot=coolant.heat(hot.getAmount()+sodium);
        if(fuelRemaining==0)fuelTotal=0;
        status=gross<requested?(room<=gross?"output_limited":"cold_limited"):"running";markForSave();
    }
    @Override protected boolean onUpdateServer(){boolean changed=super.onUpdateServer();clock();if(!structure.formed&&level.getGameTime()%20==0)structure.invalidate();react();Ports.eject(this);if(exported>0)lastOutput=exported;structure.activity(gross>0);setActive(gross>0);return changed;}
    ...
    @Override public void addContainerTrackers(MekanismContainer menu){super.addContainerTrackers(menu);
        ...
        menu.track(SyncableLong.create(()->heatSpent,v->heatSpent=v));
