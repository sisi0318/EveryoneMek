...
public final class CoronalTests{
    ...
    @GameTest(template="empty",timeoutTicks=180)
    public static void coronalPaidBatchSurvivesPauseAndDrop(GameTestHelper h){var c=SolarTests.formed(h);var m=attach(h,c);hot(c);m.autoEject=false;m.inputs.getFirst().setStack(new ItemStack(Items.RAW_IRON,64));long before=c.stored;
        CoronalMachine.processAttached(c);check(m.remaining==32&&m.batch==32&&m.progress==1&&before-c.stored==32*SolarConfig.CORONAL_ENERGY.get(),"Batch was not paid atomically");c.enabled=false;
        ...
        check(loaded.remaining==32&&loaded.progress==1&&loaded.inputs.getFirst().getCount()==32,"Saved paid work or real inventory missing");
        h.startSequence().thenIdle(3).thenExecute(()->check(m.progress==1,"Cold core advanced paid work"))
        .thenExecute(()->{
            var drops=Block.getDrops(m.getBlockState(),h.getLevel(),m.getBlockPos(),m);check(drops.size()==1&&drops.getFirst().get(CoronalContent.WORK.get()).getInt("remaining")==32,"Drop lost paid work");
            ...
            var replacement=(CoronalMachine)h.getLevel().getBlockEntity(pos);check(replacement!=null&&replacement.remaining==32&&replacement.progress==1&&replacement.inputs.getFirst().getCount()==32,"Real placement failed to restore work and inventory");
            replacement.inputs.getFirst().setStack(ItemStack.EMPTY);for(var slot:replacement.outputs)slot.setStack(new ItemStack(Items.COBBLESTONE,64));hot(c);
        }).thenIdle(43).thenExecute(()->{
            var replacement=(CoronalMachine)h.getLevel().getBlockEntity(m.getBlockPos());check(replacement.remaining==32&&replacement.progress==replacement.duration&&replacement.status.equals("output_full"),"Blocked paid products were lost");
            check(c.stored==c.capacity(),"Resumed prepaid work charged a second time");replacement.outputs.getFirst().setStack(ItemStack.EMPTY);c.enabled=false;
        }).thenIdle(2).thenExecute(()->{var replacement=(CoronalMachine)h.getLevel().getBlockEntity(m.getBlockPos());check(replacement.remaining==0&&replacement.outputs.getFirst().getStack().is(Items.IRON_INGOT)&&replacement.outputs.getFirst().getCount()==32,"Completed products could not leave a cold chamber");}).thenSucceed();
    }
