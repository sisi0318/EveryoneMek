...
public final class OrbitalTests{
    ...
    public static void gravityForgeKeepsPaidCargoThroughRealReplacement(GameTestHelper h){var c=ReactorTests.formed(h);hot(c);var m=place(h,ModuleKind.FORGE,c.getBlockPos().north(3));m.autoEject=false;link(h,m,c.getBlockPos());
        ...
        .thenExecute(()->{check(c.stored==c.capacity(),"Forge resume debited energy twice");var restored=(OrbitalModule)h.getLevel().getBlockEntity(m.getBlockPos());restored.inputs.getFirst().setStack(new ItemStack(ModuleContent.FLARE.get(),4));})
        .thenWaitUntil(()->{var restored=(OrbitalModule)h.getLevel().getBlockEntity(m.getBlockPos());check(restored.outputs.stream().filter(slot->slot.getStack().is(SolarContent.COMPRESSED.get())).mapToInt(slot->slot.getCount()).sum()==4,"Flare-only matter recipe required pellets or failed to finish");})
        .thenExecute(()->{var restored=(OrbitalModule)h.getLevel().getBlockEntity(m.getBlockPos());check(restored.paidEnergy==20_000_000_000L&&restored.duration==40&&restored.inputs.stream().allMatch(slot->slot.isEmpty()),"Matter recipe changed payment, time or input counts");check(restored.outputs.getFirst().getStack().is(ModuleContent.ALLOY.get())&&restored.outputs.getFirst().getCount()==16,"Single-input recipe stole the preceding alloy batch");c.enabled=false;}).thenSucceed();
    }
    ...
    public static void nodesConserveCargoAndStopAtFullReceiver(GameTestHelper h){var s=SolarTests.formed(h);hot(s);var sender=place(h,ModuleKind.NODE,s.getBlockPos().north(3));var receiver=place(h,ModuleKind.NODE,s.getBlockPos().west(9).north(3));link(h,sender,s.getBlockPos());
        NodeFrequencyTests.pair(h,sender,receiver);
