...
public final class NodeResourceTests {
    ...
    private static void pair(GameTestHelper h,SolarController core,OrbitalModule a,OrbitalModule b){var p=ReactorTests.player(h,core.getBlockPos().north());try{p.setItemInHand(InteractionHand.MAIN_HAND,new ItemStack(ModuleContent.LINKER.get()));use(p,core.getBlockPos());use(p,a.getBlockPos());use(p,b.getBlockPos());check(a.source!=null&&a.source.pos().equals(core.getBlockPos())&&a.peer!=null&&a.peer.pos().equals(b.getBlockPos())&&b.receiverConfigured,"Normal source -> sender -> distant receiver binding failed");p.setShiftKeyDown(true);p.getMainHandItem().use(h.getLevel(),p,InteractionHand.MAIN_HAND);check(!p.getMainHandItem().has(ModuleContent.LINK.get())&&a.peer!=null,"Clear selection also broke the world link");}finally{ReactorTests.close(p);}}
