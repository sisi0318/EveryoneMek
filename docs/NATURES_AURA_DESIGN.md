...
日期：2026-09-09。目标固定 Minecraft 1.21.1、NeoForge 21.1.241。`NaturesAura` 子目录现已实现九台机器，当前版本 0.1.9。
...
- 后续完成：灵气调控器、通用降生祭坛、工业养殖机，以及矿物凝聚室多方块。
...
通过 `IAuraChunk` 将环境和 Chemical 储罐的灵气 1:1 转移；默认上下限 1,200,000 和 2,200,000，低于下限释放、高于上限回收，在中间区间不耗电。每次先检查目标差额、储罐空间/储量、所选区块是否加载、原版灵气模拟结果和电量，再同时执行两侧转移。允许负值目标，保留原版负灵气机制。

范围模块最多 4 个，每个增加 8 格半径和一份正常耗电。范围、速度和能量升级可组合；客户端只显示同步状态，设置包仅作用于玩家当前打开且有权限、距离有效的机器。设置保存在 `control_settings` 物品组件和实体 NBT 中。

新增 5 项服务端测试，总数 35。测试中的调控器及带材料的元数据测试机器在结束后停机；环境用例还会清除之前的测试灵气点，避免原版 `BalanceEffect` 和 `SpreadEffect` 在用例之间搬运灵气。游戏中的原版扩散和平衡保持启用。

## 0.1.7 通用降生祭坛

从当前 `ANIMAL_SPAWNER_TYPE` 表中读取实体、材料数量、时间与灵气字段，调用原版 `AnimalSpawnerRecipe.makeEntity` 初始化实体。使用容量匹配处理重叠 Ingredient；检查生物数量、加载区块、碰撞和水陆空间后加工，只在 `addFreshEntity` 成功时消耗材料和灵气。生成被取消时保留已支付的 FE 进度。

工作范围使用世界坐标轴偏移、水平半径和生物数量上限；范围模块扩大最大半径。区域、模块与库存支持掉落保存，处理中间状态保存在世界 NBT。此版总计 41 项服务端测试和 7 项单元测试。

## 0.1.8 工业养殖机

按 `Animal.isFood` 为成年可配对动物提供两份食物，调用原版繁殖方法，保留幼体、冷却和 NeoForge 繁殖事件。`Animal.getLoveCause` 只在当前世界玩家列表中查找喂食者，因此在一次同步繁殖调用内临时提供专用 FakePlayer 的查询入口，并在 `finally` 中清理，不触发登录、网络连接或实体注册。

降生之灵由自然灵气原版事件产生：亲本周围 30 格至少 1,200,000 灵气，随机 1～3 个，每个消耗 800 环境灵气。机器只收集实际掉落，不额外制造。普通繁殖模式允许低灵气，并保留海龟、青蛙的怀卵；对海龟仅公开原版 `setHasEgg` 方法。美西螈吃热带鱼桶后返还水桶。此版总计 48 项服务端测试。

来源：[降生之灵事件](https://github.com/Ellpeck/NaturesAura/blob/70b93d37037f3559312834f0f42e824431090b64/src/main/java/de/ellpeck/naturesaura/items/ItemBirthSpirit.java)。

## 0.1.9 矿物凝聚室

采用 3×3×3 中空结构，侧面中心为朝外的控制器，另外 25 个外层位置接受外壳或端口。端口本身不保存库存，每次能力调用都确认对应控制器与结构仍有效；输入与输出分别代理控制器原料、能源、灵气及产物。Mek 的无侧面能量能力为只读，因此端口使用原版 `ForgeEnergyIntegration` 处理能量单位与取整，并暴露严格能量接口。支持失效后重新连接与真实 Mek 管道输出。

沿用主世界/下界矿物权重表、有效矿物标签、原版生成排除表、`oreEffect` 配置和高于 2,000,000 的环境灵气门槛。慷慨之粉在原版是可持续生效的效果粉实体，因此机器保留粉末作为催化材料。加工消耗石头/下界岩、FE 与 `max(1, 40000 - 4 × weight)` 灵气，储罐优先。过滤无效标签后抽样，使用 long 权重总和；选定产物随进度保存，配方表变化时复核。JEI 显示维度、权重、概率和成本。

新增 8 项服务端测试，总计 56 项，覆盖旋转与破损结构、暂停/保存、维度、极大权重、原版禁用配置、粉末组件、端口方向与失效能力，以及漏斗、电缆、加压管道、物流管道和箱子的实际生产线。

来源：[原版矿物生成](https://github.com/Ellpeck/NaturesAura/blob/70b93d37037f3559312834f0f42e824431090b64/src/main/java/de/ellpeck/naturesaura/chunk/effect/OreSpawnEffect.java)、[效果粉实体](https://github.com/Ellpeck/NaturesAura/blob/70b93d37037f3559312834f0f42e824431090b64/src/main/java/de/ellpeck/naturesaura/entities/EntityEffectInhibitor.java)。
