...
def generate(mek_jar):
    ...
    pairs = {
        ...
        'key.overloadcore.extreme': ('切换雷印极限抽能', 'Toggle Ward Extreme Extraction'),
        'overloadcore.ward.extreme_hint': ('按 %s 切换极限抽能；平时默认留电 %s%%。', 'Press %s to toggle extreme extraction. Normal mode reserves %s%% by default.'),
        'overloadcore.ward.shield_hint': ('逆命余雷 · 保命后挡下 %s 次伤害，最多持续 %s 秒。', 'Lingering Thunder · After a rescue, blocks %s hits for up to %s seconds.'),
        'overloadcore.ward.shield_hud': ('余雷护身：%s 次 · %s 秒', 'Thunder guard: %s hits · %s s'),
        'overloadcore.ward.mode.normal': ('雷印 · 留电护命', 'Ward · Reserve power'),
        'overloadcore.ward.mode.extreme': ('雷印 · 极限抽能', 'Ward · Extreme extraction'),
        'overloadcore.ward.extreme_on': ('极限抽能已开启，可动用设备的留存电量。', 'Extreme extraction enabled. Reserved power may be used.'),
        'overloadcore.ward.extreme_off': ('极限抽能已关闭，按设备设置留电。', 'Extreme extraction disabled. Source reserves are respected.'),
        'overloadcore.ward.pending_item': ('佩戴记录待核验，请管理员检查；物品会保留。', 'Wear record needs verification by an administrator. This item is preserved.'),
        'overloadcore.ward.state.equipped': ('已佩戴', 'Equipped'),
        'overloadcore.ward.state.absent': ('未佩戴', 'Not equipped'),
        'overloadcore.ward.state.pending': ('雷印待核验', 'Ward needs verification'),
        'overloadcore.ward.failure.idle': ('尚未触发', 'Not triggered'),
        'overloadcore.ward.failure.paid': ('保命成功', 'Rescue paid'),
        'overloadcore.ward.failure.pending': ('雷印记录缺失或冲突，物品已保留，请管理员核验。', 'Ward record missing or conflicting. Item preserved; administrator verification required.'),
        'overloadcore.ward.failure.no_source': ('范围内没有可用的自有或授权供能设备。', 'No usable owned or authorized power sources in range.'),
        'overloadcore.ward.failure.energy': ('供能设备电量或输出余量不足。', 'Insufficient source energy or remaining output capacity.'),
        'overloadcore.ward.failure.reserved': ('留电限制下电量不足，可切换极限抽能。', 'Insufficient power above the reserve. Extreme extraction can use reserved power.'),
        'overloadcore.ward.report': ('%s：%s；%s；护盾 %s 次。最近结果：%s。最近扣电 %s FE，供能设备 %s 个。', '%s: %s; %s; %s guard hits. Latest result: %s. Last debit: %s FE from %s sources.'),
        'overloadcore.ward.updated': ('雷印设置已更新。', 'Ward settings updated.'),
        'overloadcore.ward.update_failed': ('未能修改。请检查佩戴状态，或对准自己的供能设备。', 'No change. Check the equipped ward or aim at your own power source.'),
        'overloadcore.ward.admin_ok': ('已处理 %s 的雷印。', 'Ward updated for %s.'),
        'overloadcore.ward.admin_failed': ('未能处理 %s 的雷印：没有可核验原件，或标识已被使用。', 'Cannot update the ward for %s: no verifiable original, or identity already used.'),
