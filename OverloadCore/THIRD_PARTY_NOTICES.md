...
- The Thunder Ward stamp sprite is original built-in ImageGen output; its raw source and full prompt are kept in `art/source/thunder_ward-seal-v2-raw.png` and `art/thunder-ward-seal-prompt.txt`. With explicit user permission, only the painted checkerboard background was removed by `tools/prepare_ward_seal.cjs`; the stamp's RGB colors are preserved. The old bracelet artwork remains as design history. Curios supplies its existing empty-bracelet slot icon at runtime.
