...
- [Botania](https://github.com/VazkiiMods/Botania/blob/d617ef057edf7a4b4fb6c6ee6045c973a80bfb05/LICENSE.txt): Botania License; the prototype uses the NeoForge build at this fixed source commit.
...
Forbidden Mekanism uses Forbidden & Arcanus and Valhelsia Core as separate runtime dependencies. It calls their APIs and uses a narrowly scoped Clibano state accessor; it does not redistribute their implementations, JARs, models or textures. Its original machine and module art is generated with the built-in image_gen tool; prompts and references are recorded in `Forbidden-Arcanus/art/prompts.json`. Forge tier installers and compressed material blocks use original generated art; upstream item colors and silhouettes were inspected only as style references. Mekanism upgrade and bar widgets are reused from the separate runtime dependency.
...
Botanical Mekanism keeps Botania, Patchouli, Curios and Mekanism as separate runtime dependencies. It extends Botania's generating-flower API and uses narrowly scoped hooks for the bionic Jaded Amaranthus power source; the native flower implementation and texture come from the installed Botania JAR. Three new flower sprites were generated with built-in ImageGen; prompts and references are in `Botania/art/prompts.json`. The upstream Botania license is included in addon notices. No upstream JAR or native flower texture is bundled into the addon.

The mechanical apothecary uses Mekanism's runtime machine and inventory APIs and Botania's runtime recipes. Its original generated art and prompt are in `Botania/art/mechanical-apothecary.json`; the Mekanism MIT notice is included in the addon. Its JEI integration reuses Botania's existing category for native recipes and provides a separate category only for mechanical recipes.

The resonance interface references the network-selection and member-management interaction patterns of [Flux Networks, commit ea8b6e0](https://github.com/SonarSonic/Flux-Networks/tree/ea8b6e0cf708098ab2f02bf5c3d5c1f0ed2a1f5f). No Flux Networks code or GUI assets are redistributed. Its repository has an MIT code license and separately reserves GUI designs/resources in its README; the independent interface and research links are documented in `Botania/UPSTREAM.md`.
