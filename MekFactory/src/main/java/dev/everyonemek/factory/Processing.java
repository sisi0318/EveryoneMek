...
public final class Processing {
    ...
    public static final class Job {
        ...
        Job(RecipePlan p,int n){chemicalWork=p.chemicalWork==null?null:p.chemicalWork.copy();recipe=p.id;units=n;ticks=p.ticks;energy=p.energy;baseTicks=p.baseTicks;baseEnergy=p.baseEnergy;fixedEnergy=p.fixedEnergy;exponential=p.exponential;operations=p.operations;items=List.copyOf(p.outItems);fluids=List.copyOf(p.outFluids);chemicals=List.copyOf(p.outChemicals);}
        Job copy(int n){var p=new RecipePlan(recipe);p.chemicalWork=chemicalWork;p.ticks=ticks;p.energy=energy;p.baseTicks=baseTicks;p.baseEnergy=baseEnergy;p.fixedEnergy=fixedEnergy;p.exponential=exponential;p.operations=operations;p.outItems.addAll(items);p.outFluids.addAll(fluids);p.outChemicals.addAll(chemicals);var j=new Job(p,n);j.progress=progress;return j;}
    ...
    public void tick(Controller c){
        ...
        SecondaryInputs.convert(c,profile);
        var inputs=c.inputBank();var outputs=c.outputBank();
        for(var j:jobs)j.refresh(c);
        for(int cycle=0;cycle<FactoryConfig.PROCESSING_CYCLES.get();cycle++){
            // Clear completed products between work steps so fast recipes can use the next step.
            // Input reservations and the real induction provider's budget remain shared by both steps.
            if(cycle>0)Ports.eject(c);
            c.running=Math.max(c.running,advance(c,profile,inputs,outputs,c.parallel));
        }
    ...
    private int advance(Controller c,Profiles.Profile profile,ResourceBank inputs,ResourceBank outputs,int limit){
        ...
        for(int step=0;step<original&&budget>0;step++){
            ...
            Job active=j;if(n<j.units){active=j.copy(n);j.units-=n;split.add(active);}
            if(active.chemicalWork!=null)active.chemicalWork.consume(inputs,chemicalSlot,n,chemicalMultiplier);
            active.progress++;
            int used=(n+j.operations-1)/j.operations;running+=used;budget-=used;c.powerUsed=mekanism.api.math.MathUtils.addClamped(c.powerUsed,cost);c.progress=Math.max(c.progress,active.progress*100/active.ticks);c.markForSave();
        }
        jobs.addAll(split);if(!jobs.isEmpty())cursor=(cursor+1)%jobs.size();
        for(var it=jobs.iterator();it.hasNext();){var j=it.next();if(j.progress>=j.ticks&&j.deliver(outputs)){it.remove();c.markForSave();}}
        return running;
    }
    public ListTag save(HolderLookup.Provider r){var list=new ListTag();for(var j:jobs){var tag=new CompoundTag();tag.putString("recipe",j.recipe);if(j.chemicalWork!=null)tag.put("chemical_work",j.chemicalWork.save(r));tag.putInt("units",j.units);tag.putInt("ticks",j.ticks);tag.putInt("progress",j.progress);tag.putLong("energy",j.energy);tag.putInt("base_ticks",j.baseTicks);tag.putLong("base_energy",j.baseEnergy);tag.putInt("operations",j.operations);tag.putBoolean("fixed",j.fixedEnergy);tag.putBoolean("exponential",j.exponential);var i=new ListTag();j.items.forEach(s->i.add(s.save(r)));tag.put("items",i);var f=new ListTag();j.fluids.forEach(s->f.add(s.saveOptional(r)));tag.put("fluids",f);var g=new ListTag();j.chemicals.forEach(s->g.add(s.saveOptional(r)));tag.put("chemicals",g);list.add(tag);}return list;}
    public void load(ListTag list,HolderLookup.Provider r){jobs.clear();for(var value:list){if(jobs.size()>=512)break;var tag=(CompoundTag)value;var p=new RecipePlan(tag.getString("recipe"));if(tag.contains("chemical_work"))p.chemicalWork=ChemicalWork.load(tag.getCompound("chemical_work"),r);p.ticks=Math.clamp(tag.getInt("ticks"),1,10000000);p.energy=Math.max(1,tag.getLong("energy"));p.baseTicks=tag.contains("base_ticks")?Math.clamp(tag.getInt("base_ticks"),1,10000000):p.ticks;p.baseEnergy=tag.contains("base_energy")?Math.max(1,tag.getLong("base_energy")):p.energy;p.operations=Math.clamp(tag.getInt("operations"),1,65536);p.fixedEnergy=tag.getBoolean("fixed");p.exponential=tag.getBoolean("exponential");
