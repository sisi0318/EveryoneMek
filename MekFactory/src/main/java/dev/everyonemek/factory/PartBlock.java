...
public final class PartBlock extends BaseEntityBlock {
    public enum Kind {FRAME,CASING,GLASS,PORT,CONVERTER}
    ...
    @Override public void appendHoverText(ItemStack stack,net.minecraft.world.item.Item.TooltipContext context,java.util.List<net.minecraft.network.chat.Component> tooltip,net.minecraft.world.item.TooltipFlag flag){
        if(kind==Kind.CONVERTER){tooltip.add(Content.text("conversion_hint"));tooltip.add(Content.text("conversion_io"));}
    ...
    @Override protected ItemInteractionResult useItemOn(ItemStack stack,BlockState state,Level level,BlockPos pos,Player player,InteractionHand hand,BlockHitResult hit){
        if((kind==Kind.PORT||kind==Kind.CONVERTER)&&stack.canPerformAction(mekanism.api.MekanismItemAbilities.WRENCH_CONFIGURE))return ItemInteractionResult.SKIP_DEFAULT_BLOCK_INTERACTION;
