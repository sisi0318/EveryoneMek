...
public final class FactoryMenu extends MekanismTileContainer<Controller> {
    ...
    public int jobIndex,jobCount,jobUnits,jobProgress,jobTicks=1,availableParallel;
    ...
    public FactoryMenu(int id,Inventory inv,Controller c,BlockPos clicked){
        ...
        track(SyncableInt.create(()->{int n=Math.min(c.parallelLimit,c.structure.parallel);return FactoryConfig.MACHINES_LIMIT_PARALLEL.get()?Math.min(n,c.template.getCount()):n;},v->availableParallel=v));
    }
