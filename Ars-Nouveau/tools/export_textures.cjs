...
const machines = ['source_generator', 'source_converter', 'imbuement_chamber', 'enchanting_apparatus',
  'source_extractor', 'magic_crusher', 'glyph_scribe', 'potion_mixer', 'potion_bottler',
  'drygmy_station', 'whirlisprig_station', 'ritual_controller'];
const faces = {front: [0, 0], top: [1, 0], side: [0, 1], front_active: [1, 1]};

async function pixelFace(machine, face, transform, shade) {
  const file = path.join(root, 'src/main/resources/assets/arsmekanism/textures/block', machine, `${face}.png`);
...
async function renderPreview() {
  const titles = ['通用魔源发生器', '通用魔源转换器', '通用魔源灌注室', '通用魔源附魔装置',
    '通用魔源萃取机', '通用魔法粉碎机', '通用魔符抄写机', '通用药水混合器', '通用药水灌装机',
    '通用德格米收获站', '通用风转草培育站', '通用仪式控制器'];
  const subtitles = ['电力转化 · 魔源供给', '魔源转移 · 双向运输', '材料灌注', '物品合成 · 装备附魔',
    '食物 · 燃料 · 余热', '原料粉碎', '魔符制作', '药液混合', '药液装填与回收',
    '德格米 · 供魔与收集', '风转草 · 供魔与收集', '农业仪式自动化'];
  const content = [];
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
    ...
    <g font-family="Microsoft YaHei,Arial,sans-serif" fill="#24302b">
      <text x="48" y="46" font-size="25" font-weight="700">Ars Mekanism</text>
...
async function main() {
  for (const machine of machines) {
    ...
    const output = path.join(root, 'src/main/resources/assets/arsmekanism/textures/block', machine);
  ...
  for (let row = 0; row < machines.length; row++) {
    for (const [column, face] of Object.keys(faces).entries()) {
      images.push({
        input: await sharp(path.join(root, 'src/main/resources/assets/arsmekanism/textures/block', machines[row], `${face}.png`))
