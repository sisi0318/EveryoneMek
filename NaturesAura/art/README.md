...
| 灵气装瓶机 | 灌装喷嘴、浅灰玻璃瓶轮廓、绿色瓶内指示 |
| 灵气调控器 | 绿色与蓝色双向箭头、环境回收和释放指示 |
| 通用降生祭坛 | 浅灰卵形培育核心、青色电极、封闭工业舱 |
| 工业养殖机 | 双动物指示面板、棕色饲料托盘、粉色繁殖状态灯 |
| 矿物凝聚室 | 小型矿石舱、金色矿点、绿色工作条与加固顶板 |

每台机器包括 `front.png`、`top.png`、`side.png`、`front_active.png`，均为真正的 16×16 PNG。侧面贴图同时用于背面与底面。运行时通过模型状态切换正面贴图，不额外加载高分辨率原稿。

矿物凝聚室外壳复用控制器加固顶板，输入/输出端口分别使用控制器的静止/亮起工作面，不增加高分辨率运行时贴图。对应原稿与提示词见 [ore-prompt.json](ore-prompt.json)。

![方块预览](block-preview.png)

原稿位于 `source/`，初始四台机器的提示词与原版参考来源见 [prompts.json](prompts.json)，装瓶机见 [bottler-prompt.json](bottler-prompt.json)，调控器见 [controller-prompt.json](controller-prompt.json)，降生祭坛见 [spawner-prompt.json](spawner-prompt.json)，养殖机见 [breeder-prompt.json](breeder-prompt.json)。原稿中的四个区域依次为左上正面、右上顶部、左下侧面、右下工作正面。`texture-sheet.png` 的各行依次对应上表，四列顺序与上述区域一致。
