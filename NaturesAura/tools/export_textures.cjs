...
const machines = ['universal_aura_generator', 'universal_forest_ritual', 'universal_natural_altar', 'universal_offering', 'aura_bottler', 'aura_controller', 'universal_animal_spawner', 'industrial_breeder'];
...
async function renderPreview() {
  const titles = ['通用灵气发生器', '通用森林仪式', '通用自然祭坛', '通用呼唤仪式', '灵气装瓶机', '灵气调控器', '通用降生祭坛', '工业养殖机'];
  const subtitles = ['电力转化 · 绿色能量条', '树苗 · 金叶粉 · 八方原料', '灵气灌注 · 催化接口', '供品托盘 · 呼唤指示灯', '灌装喷嘴 · 玻璃瓶 · 模拟环境', '双向箭头 · 回收与释放', '生物培育舱 · 区域与数量控制', '饲料托盘 · 原版繁殖 · 降生之灵'];
  const content = [];
