...
**控制器只作为炉体的一部分工作，不再支持炉外远程连接。** 成型后，右键控制器、端口或任一炉壳，均打开同一个控制器界面并遵循 Mek 安全设置。未安装控制器的原炽炉仍使用原界面。
...
管道和电缆连接端口朝向炉外的一面。端口的物品模式由控制器六面配置决定：“输入”处理原料，“额外”和“输入 2”都直接连接界面中的**同一个灵魂槽**，“输出”处理成品。开启自动弹出后，输出端口可向箱子或 Mek 物流管道出料；FE 遵循对应面的能量设置。配置窗口用端口图标标出可接线的结构面，端口灯亮表示已连接完整炉体。
...
旧侧面控制器可继续作为炉体部件工作；若要移到正面，可拆下、补回侧面炉砖，再安装到核心位置并重新激活。**旧炉外控制器需移入炉体才能继续加工**，库存、储能、升级和设置保留。旧灵魂缓冲会并入唯一灵魂槽，多余灵魂及遗留燃料退回输出；空间不足时原样保留，腾出空间后继续整理。
...
17 项无界面服务端 GameTest 与 5 项字节码契约检查覆盖现有加工、资源和存档，以及炉体全部 26 个外部方块统一打开界面、权限拦截、唯一灵魂库存、产物即时输出、多类残渣容量、旧远程连接失效与物品保留、真实端口和 Mek 管道。界面槽位及坐标已核对；客户端视觉及整合包体验由玩家在游戏内验收。
...
Use a Clibano Controller directly on the Clibano Core or the front center of an assembled 3×3×3 furnace. You may also place it in the core position while building, then use Mundabitur Dust on its front to activate the complete shell. The controller must be part of the furnace; remote connections have been removed. Right-clicking any casing or port opens the same controller menu with Mek security checks. The interface has one material grid, one enhancer slot, one soul slot and one output group. Ports access that same soul slot directly; products and residue outputs arrive immediately in the output group. A horizontal soul bar shows remaining duration, and the processing bars show the active lanes. Embedded parts retain the matching native shell models and resource-pack support.
