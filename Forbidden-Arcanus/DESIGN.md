...
0.2.5 支持炽炉核心直接替换、连续炉壳纹理和 FE 加热。16 项服务端验收与 5 项字节码契约检查覆盖实现。玩家说明以 [README.md](README.md) 为准，源码入口及维护要求见 [AGENTS.md](AGENTS.md)。客户端视觉由玩家验收。

## 架构取舍

赫菲斯托斯锻造室放在原 9×9 地面平台中心，原料、增强器、等级和四种资源由 Mek 机器保存。用户已确认内部加工、取消锤子、取消真实基座摆料，并明确要求删除旧实现、不做 0.1.0 迁移。

炽炉控制器可直接替换正面核心，也兼容侧面、背面及旧炉外绑定。控制器支付 FE 提供加热，原中心实体继续处理双槽加工、合金、火焰、残渣与经验。独立端口转发控制器资源和六面设置。两种机器复用 Mek 的能量、库存、安全、红石和升级，端口弹出复用 Mek 的物品传输请求。
...
`Binding` 仅支持原炽炉。绑定核对 8 格距离、原机身份、单控制器认领、完整结构、已加载区块与访问权限。菜单通过原索引访问库存，隐藏不再使用的燃料槽，仅显示其余六槽；每次操作重新校验，Shift 点击返回玩家背包。

嵌入模式允许四个水平面的中央，包括核心位置。`ClibanoEmbedding` 包装原 `BlockPattern.getPattern()` 的中心格，原 C 核心格仅放宽为本控制器。`MachineBlock.useItemOn` 在打开 Mek 菜单前处理洁净粉末，交给原激活物品；校验完整炉壳、点击朝向、权限和唯一控制器。成型时跳过本模组部件，其余变化由原组装执行。真实 `BlockItem.place` 处理原核心与成型炉壳替换，返还原核心／砖块；成型安装上下文仅阻止指定炉壳拆除原中心，并在 finally 清理。
...
`ClibanoAutomation` 经原缓存选配方、供料和收取，不再要求燃料。两格补给槽均输送灵魂。旧控制器／原机中的燃料按容量原样退到输出，不能清空；现有 16 槽顺序和物品附件保留。经验按钮继续调用原一次性领取方法。参见 [炽炉核心][clibano-main]、[炽炉配方][clibano-recipe]、[残渣][residues]。

`ClibanoHeatingMixin` 在原配方更新后确认可加工、有效认领和能量，支付默认 50 FE／有效 tick；能量升级提高效率，原配方时长不变。仅在该次原 tick 中使用临时热状态，WrapMethod 的 finally 恢复旧燃烧时长。原 tick 的燃料读取视为空，因此不消耗煤炭；加工 tick 在无电、暂停或不可推进时跳过，保留进度。原灵魂计时与残渣转换仍运行。未接控制器的炽炉继续使用原燃料逻辑；断开的认领不会强制加载控制器区块。

`ClibanoControllerBlock.shell` 和端口 formed／connected 状态选择原 side、top 与四种 front 模型；只在中央采样已有原创端口灯的一个像素，形成两像素标记。边缘完整沿用原模型和纹理，不使用近似底色重画外框。所有运行 JSON 由生成器维护，依赖图稿只在本地预览，不进入公开资源或 JAR。

新锻造室中仅保留原料、增强器、资源、输出、状态、配方选择与 Mek 通用设置。正常锻造和炽炉继续进入原 JEI 分类；等级插件属于原工作台分类，不创建重复说明面板。

## 验收

16 项 GameTest 覆盖锻造室、资源插件、实际合成和存档，以及四朝向核心／侧面安装、方块右键激活、FE 加热扣费与断电保进度、旧燃料回收、五端口与 Mek 管道出料、损坏修复。5 项字节码检查验证 Mek 升级、原炉组装／移除／电热钩子和客户端六面图标入口。
