...
final class ClibanoIntegrationContractTest {
    ...
    @Test void nativeFormationAndRemovalHooksStillMatchPublishedRelease() throws IOException {
        ...
        assertTrue(frame.methods.stream().anyMatch(m -> m.name.equals("useWithoutItem") && m.desc.equals(
    ...
    @Test void electricHeatWrapsTheNativeRecipeTickAndFuelRead() throws IOException {
        ...
        String recipeArgs = "(Lnet/minecraft/world/item/crafting/RecipeHolder;Lcom/stal111/forbidden_arcanus/common/block/entity/clibano/ClibanoInputSlot;)";
        assertTrue(main.methods.stream().anyMatch(m -> m.name.equals("canSmelt") && m.desc.equals(recipeArgs + "Z")));
        assertTrue(main.methods.stream().anyMatch(m -> m.name.equals("finishRecipe") && m.desc.equals(recipeArgs + "V")));
        assertTrue(main.methods.stream().anyMatch(m -> m.name.equals("consumeSoul") && m.desc.equals("(Lnet/minecraft/world/level/Level;)V")));
        var residues = read("com/stal111/forbidden_arcanus/common/block/entity/clibano/ResiduesStorage");
        var residueTick = residues.methods.stream().filter(m -> m.name.equals("tick") && m.desc.equals("(L" + mainName + ";)V")).findFirst().orElseThrow();
        int iterations = 0;
        for (var instruction : residueTick.instructions) if (instruction instanceof MethodInsnNode call
              && call.owner.equals("it/unimi/dsi/fastutil/objects/Object2IntOpenHashMap") && call.name.equals("forEach")
              && call.desc.equals("(Ljava/util/function/BiConsumer;)V")) iterations++;
