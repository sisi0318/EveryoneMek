...
public final class CorporeaFlower extends BlockEntity {
    ...
    public void saveItem(CompoundTag tag) { tag.putInt("bridge_mode", mode); }
    ...
    @Override protected void saveAdditional(CompoundTag tag, HolderLookup.Provider provider) {
        super.saveAdditional(tag, provider); saveItem(tag);
    ...
    @Override protected void loadAdditional(CompoundTag tag, HolderLookup.Provider provider) {
        super.loadAdditional(tag, provider); loadItem(tag); connection = tag.getCompound("me_connection").copy();
    }
}
