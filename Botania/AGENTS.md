...
- 当前为 **0.1.0-alpha.30 可运行原型**，包 `dev.everyonemek.botania`、模组 ID `botanicalmekanism`、产物 `BotanicalMekanism-<版本>.jar`。已增加配方驱动的魔力温室，并实现设计 P1–P3 主线：导能莲、六种仿生功能花、两种辅助设备及十种加工／控制设备，接入原生火花与 Chemical 魔网；旧共鸣网络兼容保留。跨维度和后续候选花不在本版。使用说明以 README 为准。
...
- 客户端游戏验收由用户进行，不运行客户端。alpha.30 已通过 64 项 Appbot 共存服务端 GameTest 和既有单元检查，包含原温室行为及新增规则扩展回归；此次没有重跑其他依赖组合，不代表客户端视觉验收。

## alpha.30 保留运行模式，注册式扩展

- 用户最终明确“保留我们原本的运行模式，再融合 ExtraMachinery 思路”。只采用统一规则接口／注册表，不引入统一 20 tick、固定 1.8 倍食物收益或取消原冷却。八种花的原有数值、耗时、搭配、颜色顺序和数据保存保持。
- GreenhouseFlowerRule 统一原料判断、resolve 返回值、可选流体用量、prepare／onBlocked 状态回调，以及 GUI／JEI 的描述和变化收益标记。BuiltinGreenhouseRules 中每种花独立实现；GreenhouseNative 保留原辅助入口，分发到注册规则。
- GreenhouseRules 接受命名空间 ID，以不可变映射保存；重复 ID／保留 fixed 拒绝注册。扩展在两端 FMLCommonSetupEvent.enqueueWork 注册，配方加载早于注册属于错误。原 endoflame 等短名仍映射到 botanicalmekanism 命名空间；默认 JSON 与旧存档不改。
- GreenhouseRecipe 移除固定 FORMULAS 白名单，用注册表校验。Work、流体口、Screen 和 JEI 不再按花名写分支；固定数据包配方仍走原多材料匹配，动态规则为一物品或一份流体。执行器校验产量、时间、冷却以及返回花的类型和数量，避免扩展规则意外消耗或替换花。
- 规则对象共享，结果只写入花副本，禁止把单花历史放在规则对象字段中；解析结果必须确定且不修改世界，原计划缓存和重载失效判定继续使用。prepare／onBlocked 仅接受保留物品种类与数量的返回值；签名格式及库存索引均未改变。
- recipeNote／variableOutput／previewFlower 由规则决定 JEI 展示，保留变化收益不作为固定 AE 输出的约束；statusInfo 供原界面展示需要的羊毛。规则类只使用公共 Component，不引入客户端类。
- 协议升至 10，formula 同步上限由 32 扩至 256 字符。新增一个服务端集成回归：测试扩展在公共初始化中登记长命名空间流体规则，经真实 JSON 配方、网络编解码、机内流体输入、满仓、状态回调和完整产魔结算通过。原 63 项及新增一项共 64 项通过；测试类和配方不打入 JAR。
...
- 用户最新明确要求新版盘容量太少，所有盘都扩容；这取代 alpha.26／27 新盘保留原始小容量的选择。普通、便携与本模组新旧盘统一为每 k=1024、每物理字节 8000 魔力，五档容量 8,192,000／32,768,000／131,072,000／524,288,000／2,097,152,000。最大档仍在 Botania 原 int 魔力接口范围内。
- ManaCellTier.CELL_MANA_PER_BYTE 专用于物理存储密度，MANA_PER_BYTE 仍是 500 的 AE 资源核算密度；基础操作 500、每池 1,000,000、通用槽 10,000 与耗电不变。不要为扩容恢复全局 ManaKeyType 密度 Mixin，避免把合成核算与传输行为一起改掉。
- ManaCellCapacity.maximum/bytes/usedBytes 统一计算 Appbot ManaCellInventory 的实际容量和占用；普通与便携盘不再走不同的历史容量分支。自有盘直接使用 tier.capacity，Appbot tooltip 与库存使用同一计算。IManaCellItem 的原声明保留，变更集中在 StorageCell 实际存取与字节显示。
- legacy_mana_capacity 组件继续注册并随已有 ItemStack 保存，避免旧物品反序列化失败；不再写入或用于判定／限制容量，也不再显示旧版容量提示。已有魔力、升级与其他组件不清空、不缩放。超出连扩容上限都容不下的异常存量仍只拒收并允许提取，避免负数插入。
- 现有 Appbot 回归覆盖十种新盘的全量插入与满盘、普通／便携旧标记重载、字节占用、旧截图量继续补货、异常超量和销毁卡。独立 AE 回归检查全部五档及旧组件保存。

## alpha.27 旧盘容量迁移修正（alpha.28 已统一扩容）
...
## alpha.26 以 Applied Botanics 为魔力标准（容量选择已被 alpha.28 替代）

- 本阶段用户曾要求容量跟随 Appbot 原值，后于 alpha.28 明确要求全部扩容。此时新盘曾为每 k=1000 字节、每字节 500 魔力，五档 500,000／2,000,000／8,000,000／32,000,000／128,000,000；删除的全局 ManaKeyType 密度覆盖仍不恢复，当前物理盘容量规则见顶部 alpha.28。
