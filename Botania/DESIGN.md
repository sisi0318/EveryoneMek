...
**实现状态：0.1.0-alpha.23 已接入 P1–P3 主线**：两种资源辅助设备、十种加工／控制设备、导能莲、六种仿生功能花，以及原生火花距离升级与旧共鸣设备兼容。可用功能、支持范围及实际参数见 [README](README.md)。本文保留原提案和后续候选，主线原型不代表所有第三方配方或客户端视觉已经验收。
...
- 导能莲单独制作原创花形模型与 16×16 材质，允许少量科技细节；六种仿生功能花引用原生花模型。普通机器按用户最新要求改为 Botania 原材质的开放式装置模型，清单见 art/botanical-machine-models.json。
...
## 13. alpha.7 最新决定：复用原火花

用户确认无需独立网络核心、选网、成员与优先级。新路线为原池和机器安装同色原火花，双端共鸣增幅器将各轴范围扩大至 32 格。沿用原火花角色、传输与费用，不再建立第二套新网络。旧共鸣花／芽退出新合成及创造列表，旧设备继续兼容。

用户特别要求共鸣花模型保留，后续另有用途。因此保留模型、贴图、原稿和注册，不进行破坏性迁移。机器外观同时改为活石／活木装置、花药碗、符文台、凝聚盘和酿造架，沿用原 Botania 材质；这替代旧工业方块外观提案。


## 14. alpha.8 词典、织网花和扩展入口

全部新增设备与花进入原词典“植物机械”分类，右键设备可直接阅读，机械花药配方按实际数据画成物品格。用户确认织网花需双向互通，因此实现 ME 存储挂载和多媒体节点视图，复用普通／主火花的原网络，不引入另一套选择网络流程。

仿生织网花由 ME 电力维持，1 通道、4 AE/t，物品仍在原库存；循环请求排除原多媒体网络的挂载，多网络共享 ME 的场景已有真实服务端回归。AE2 保持可选，未装时不加载其类。细节见 CORPOREA.md。

alpha.9 已实现九格样品筛选、通过整次原多媒体请求的缺额提交 AE 合成，以及七类设备的 JEI 一键填充。成品回到 ME，由原装置或保持器重试取货，任务世界保存而不复制到拆装物品。库存节点定位诊断仍是待实现项；库存显示和普通物品访问优先复用 AE 现有终端／监控能力，不为相同用途再堆机器。

## 15. alpha.10 更新

充能座新增魔力罐，接入普通火花、管道与 ME 总线；侧面配置以“魔力”显示实际输入输出。AE 新增独立魔力类型和一池容量的存储盘。盘使用本模组组件保存数值，AE 为可选依赖。词典和提示改为面向玩家的操作说明，开发契约见 AGENTS.md。

## 16. alpha.11 更新

储魔机器按六面输入配置从相邻池取魔力，共用每 tick 1000 的直接抽取上限。JEI 新增魔力材料，并通过 AE2 JEI Integration 接入样板加号和拖放；酿造按容器匹配成本与成品。输入/输出面兼收辅料，实际合成 CPU、供应器和风之符文加工已有集成验证。


## 17. alpha.16 使用反馈

纯净改为八槽、每轮最多八件，完整周期按原 time×8；灌注改为八槽、20 tick 一轮、逐件魔力消耗。少量材料不等待凑满，混合材料按适用配方处理。扩展槽位追加，保留原电池／输出索引。

灌注内部催化器优先，空槽读取底部实物，并用原池的脉动符号显示。发射器、法杖和侧面配置接入原生流程；魔力只保留一条图条。原模型不重设计，通过裁去共面和内部面修复闪烁。Applied Botanics 的网络池以 SafeMana 实际存取量联动，不另存一份 ME 魔力。

## 18. alpha.17 机械火花与 ME 魔力统一

机械火花直接沿用原 ManaSparkEntity。每组同维度、同色的相连机械火花只保留一个机械主火花，范围与效率升级存放在它的两个实体槽内，成员只共享效果。范围 12～76 格、速度 1～9 倍；不增加第二种流向配置。原火花升级仍决定具体传魔方式，普通火花保留原来的距离。

主火花冲突时停用共享加成，保留原有行为；网络拆分后只给仍连接主火花的成员加成。离线不加载区块，不补发流量。旧共鸣花与芽模型、存档继续保留。

用户否定分别制作两套魔力盘的使用负担。安装 Applied Botanics 时统一为它的原 ME 魔力类型，现有盘、总线、样板供应器与福鲁池直接互用。旧类型名通过注册表别名解码到同一类型。Appbot 盘保留原物品及存量，同档容量对齐到现有五档；无需销毁或重做旧盘。

## 19. alpha.23 火花承载 ME 频道

用户要求打磨并实现，频道数量高于早先 8／32 的建议。最终为主火花共享 1～4 个 ME 频道模块，基础容量 32／64／128／256，沿用范围升级。ME 电缆和控制器提供安装点；主火花作为无线树根，其他 ME 火花为分支或中继。魔力池上的机械火花仍做原魔力工作，不作为 ME 中继。

通过 AE 原 GridHelper 建立真实连接，设备、库存、合成 CPU 和供电都由 AE 管理，不复制服务或计数。容量定点扩展仅属于火花创建的节点和连接；普通细线 8、致密线 32 保留。因此高容量入口直接安装在控制器上，出口拆成多处分支。默认整合包下已用 256 台真实接口验证四档实际激活数量。

只连接一个同色机械主火花管理的组件，按稳定位置顺序构建树；有电缆已连通的分支不再加无线边，两个独立控制器网不合并。每组最多 128 个 ME 接入火花，不强制加载区块。主火花耗电 4+容量/32 AE/t，每条无线边在子节点登记 8+ceil(欧氏距离/16)+容量/32 AE/t，由 AE 原能量服务扣除。断电后拆线并短暂等待重试，网络重新寻路期间不反复拆线。

新槽追加到原两个实体升级槽之后；菜单保留原玩家槽索引，把频道槽追加为 38。世界实体保存 AE 节点状态，拆装物品只保存实际升级，不复制网络节点数据。老世界缺失第三槽时补空。
