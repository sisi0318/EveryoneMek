...
**已确定：由一款专用仿生产能花将 FE 转为魔力，不使用 Mek 机器产魔力；保留原生魔力互通和仿生功能花。专用产能花采用原创花形，可加入科技元素；已有功能花的仿生版本保留对应原模型。所有仿生花均不要求种在草地或泥土上。** 本文其余名称、数值与开发顺序是建议方案，尚未实现。所有上游事实的源码入口见 [UPSTREAM.md](UPSTREAM.md)。
...
| --- | --- | --- |
| 魔力网络 | 专用仿生花接 FE 产魔力，经原发射器与池汇集，再接 Mek 管道 | 原产能花、发射器、魔力池和火花仍可使用；互通器仅转移资源 |
...
以下共建议 **10 种加工设备**，另有互通器、充能座两种资源辅助设备，以及专用导能莲和六种仿生功能花。不设置机器式魔力发生器，也不要求一次做完。
...
| 阶段 | 实际内容 | 结束条件 |
| --- | --- | --- |
| P0：验证设计可实现 | 固定 Botania JAR；导能莲 → 原发射器／池 → 互通原型；一种仿生功能花 | 非土壤安装、电魔力守恒、单 tick 产率、原花模型与保存契约明确 |
| P1：基础生产线 | 导能莲、互通器；花瓣、灌注、符文、纯净四种加工机 | 原花可供启动所需魔力；导能莲阵列通过实际入池产量和平衡验证 |
| P2：仿生与进阶加工 | 六种仿生功能花；泰拉、酿造、凝矿、异构；充能座 | 原生对照、随机与组件保存、循环能量检查通过 |
...
建议从 **导能莲供魔网络＋四台基础加工机** 起步，同时验证一种仿生功能花，再扩充其余五种。完整候选为 2 种资源辅助设备、10 种加工／控制设备，以及 1 种专用产能花和 6 种仿生功能花；这是分阶段目录，不是一次性实现承诺。

用户已确定花式产能、花形可带科技元素、全部仿生花不受土壤限制，无需再次询问。后续评审集中在导能莲名称、原创花形、50 FE／魔力与单株 4 魔力／tick 的实测平衡，以及进阶加工范围。实施前完成 P0，给出实际 JAR 和关键适配方式的可运行证据。
