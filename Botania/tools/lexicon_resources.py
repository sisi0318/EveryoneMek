...
HELP['mana_storage_cell'].append(('安装 Applied Botanics 时，使用它的原生魔力盘。普通盘、便携盘和旧扩展盘均使用扩容后的容量，已有魔力保留。悬停提示会显示实际魔力与容量。', 'With Applied Botanics installed, use its native mana cells. Normal, portable and existing addon cells share the expanded capacities and retain their contents. Tooltips show actual mana and capacity.'))
