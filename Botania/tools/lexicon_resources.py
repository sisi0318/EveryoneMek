...
HELP = {
    ...
    'mana_bridge': [('$(item)魔力互通器$(0)能在魔力池和加压管道之间输送魔力。将它贴着魔力池放下，选择连接方向，再选择抽取或供给。$(p)需要电能，每秒最多输送 20,000 魔力。连接池的一面不能再接管道。', 'The $(item)Mana Bridge$(0) moves mana between a pool and pressurized tubes. Place it beside a pool, select that side and choose draw or supply.$(p)It needs electricity and transfers up to 20,000 mana per second. The pool face cannot also connect to a tube.')],
...
def generate(root, write, zh, en, plants, recipes):
    ...
    for index, (name, descriptions) in enumerate(HELP.items()):
        ...
        if name in mechanical:
            ...
            pages.append({'type': 'patchouli:crafting', 'recipe': item, 'flag': 'mod:ae2'})
