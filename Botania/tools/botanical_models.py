...
"""
import json
from model_surfaces import exterior, validate
...
def models():
    ...
    shapes['mana_greenhouse'] = base() + [box((1, 4, 1), (15, 6, 15)),
        box((1, 6, 1), (2, 15, 2), 'wood'), box((14, 6, 1), (15, 15, 2), 'wood'),
        box((1, 6, 14), (2, 15, 15), 'wood'), box((14, 6, 14), (15, 15, 15), 'wood'),
        box((1, 15, 1), (15, 16, 2), 'metal'), box((1, 15, 14), (15, 16, 15), 'metal'),
        box((1, 15, 2), (2, 16, 14), 'metal'), box((14, 15, 2), (15, 16, 14), 'metal'),
        box((2, 6, 1.25), (14, 15, 1.5), 'glass'), box((2, 6, 14.5), (14, 15, 14.75), 'glass'),
        box((1.25, 6, 2), (1.5, 15, 14), 'glass'), box((14.5, 6, 2), (14.75, 15, 14), 'glass'),
        box((2, 15.25, 2), (14, 15.5, 14), 'glass'), box((6, 4, 0), (10, 6, 1), 'indicator')]
    return shapes
...
def generate(root, write):
    ...
    for name, elements in definitions.items():
        model = {'parent': 'minecraft:block/block', 'render_type': 'minecraft:cutout', 'ambientocclusion': True,
                 'textures': {**PALETTE, 'particle': '#stone', 'indicator': PALETTE['metal']}, 'elements': exterior(elements)}
        if name == 'mana_greenhouse': model['textures']['glass'] = 'minecraft:block/glass'
    ...
    manifest = {'version': 'alpha.29', 'method': 'Original cuboids with covered faces removed; native Botania material references. Collision stays on the original solids.',
                'palette': {**PALETTE, 'glass': 'minecraft:block/glass'}, 'models': {name: {'elements': len(exterior(elements)), 'solid_elements': sum('rotation' not in e for e in elements)} for name, elements in definitions.items()}}
