...
def generate(root, write):
    ...
    manifest = {'version': 'alpha.16', 'method': 'Original cuboids with covered faces removed; native Botania material references. Collision stays on the original solids.',
                'palette': PALETTE, 'models': {name: {'elements': len(exterior(elements)), 'solid_elements': sum('rotation' not in e for e in elements)} for name, elements in definitions.items()}}
